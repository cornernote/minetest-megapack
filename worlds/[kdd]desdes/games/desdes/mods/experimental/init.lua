...
--TESTING STUFF BY KDDEKADENZ

minetest.register_entity("experimental:bal", {
	-- Static definition
	hp_max = 2000,
	physical = true, -- Collides with things
	-- weight = 5,
	...
	on_punch = function(self, hitter)
		local direct = hitter:get_look_yaw()
		minetest.chat_send_all(direct)
		if direct<2 and direct>0 then
		self.object:setvelocity({x=0, y=2, z=1})
		end
		if direct<1 and direct>-1 then
		self.object:setvelocity({x=1, y=2, z=0})
		end
		if direct<-1 and direct>-2 then
		self.object:setvelocity({x=0, y=2, z=-1})
		end
		if direct<-2 and direct>-3 then
		self.object:setvelocity({x=-1, y=2, z=0})
		end
	end,
})
