...
-------------------------------------------------------------------------------
-- name: animals_spawn_in_shadows(animal)
--
...
function animals_spawn_in_shadows(animal_name,animal_transform,spawning_data,environment) 

	print("\tregistering shadow spawn abm callback for animal "..animal_name)
				...
				if animal_name == nil then
					...
					if node_above.name == "air" then
						...
						if spawning.spawned_around(animal_name,animal_transform,pos,spawning_data.density) == false then
							...
							if animals_max_light_around(pos,3,0.5) < 3 then
								...
								print("Spawning "..animal_name.." in shadows at position "..printpos(pos))
							end
						end
					end
				end
...
animals_spawn_algorithms["shadows"] = animals_spawn_in_shadows